...
interface ResolutionProps {
    onBack: () => void;
    onNext: (resolution: StreamResolution) => void;
...
const RESOLUTION_OPTIONS: StreamResolution[] = [
...
const resolutionLabel = (resolution: StreamResolution) => `${resolution.width}x${resolution.height}`;

export class ResolutionForm extends Component<ResolutionProps, ResolutionState> {
    constructor(props: ResolutionProps) {
        super(props);

        const defaultOption = RESOLUTION_OPTIONS.find((option) =>
            option.width === this.props.defaultResolution.width &&
            option.height === this.props.defaultResolution.height
        );

        this.state = {
            selectedResolution: defaultOption ?? this.props.defaultResolution
        };
    }

    handleSelectChange = (event: any) => {
        const selected = RESOLUTION_OPTIONS.find((option) => option.label === event.target.value);
        if (!selected) {
            return;
        }

        this.setState({ selectedResolution: selected });
    };

    render() {
        return (
            <div style={formContainerStyle}>
                <h3>Stream Resolution</h3>
                <div className="mb-3">
                    <select
                        className="nvidia-dropdown"
                        id="resolutionSelect"
                        value={resolutionLabel(this.state.selectedResolution)}
                        onChange={this.handleSelectChange}
                    >
                        {RESOLUTION_OPTIONS.map((resolution) => (
                            <option key={resolution.label} value={resolution.label} className="nvidia-dropdown-option">
                                {resolution.label}
                            </option>
                        ))}
                    </select>
                ...
                <button type="button" className="nvidia-button" onClick={() => this.props.onNext(this.state.selectedResolution)} style={nextButtonStyle}>Next</button>
            </div>
        )
    }
}
